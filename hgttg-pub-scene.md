...
**Limn v4:**
```limn
for say : plnt end @ 12 mom
```

**Breaking down:**
- `plnt end` = planet ending
...
**Limn v4:**
```limn
for say tru | vog fle dem plnt for byp @ (hpr*spa)
```

**Breaking down:**
- `say tru` = says yes/truly
- `vog fle` = Vogon fleet
- `dem plnt` = demolish planet
...
**Limn v4:**
```limn
for say : i nu fro gui | i fro sma plnt ner bet
```

**Breaking down:**
- `nu fro gui` = not from Guildford (proper name abbreviated)
- `fro sma plnt ner bet` = from small planet near Betelgeuse
...
**Limn v4:**
```limn
for say : 6 cup bee mor | plnt end now
```

**Breaking down:**
- `6 cup bee mor` = six cups beer more (another round)
- `plnt end now` = planet ends now
...
**Limn v4:**
```limn
drk bee | bee goo | plnt : end^0 (now)
```

**Breaking down:**
- `drk bee` = drink beer
- `bee goo` = beer good
- `plnt : end^0 (now)` = planet ending at 0% right now (hasn't ended yet)
...
- `bee` (beer), `cup` (vessel), `drk`/`dri` (drink), `hom` (home), `plnt` (planet),
