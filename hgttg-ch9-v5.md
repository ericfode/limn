...
```limn
bel : hom mak wrl | anc mac | slp
```

- `hom mak wrl` = homes of planet-making = workshops
...
```limn
Slartibartfast say : cra wrl | sa mak sea*lan
```

- `cra wrl` = crafting planets = planet designer
...
```limn
Arthur : und^0 | cus mak wrl | sea val : rew | sur^0.95
```
...
- Planet designer = `cra wrl` (craft planet)
