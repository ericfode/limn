...
def translate_limn(thought: str) -> str:
    ...
    prompt = (
        ...
        "Core operators: ~ delegation/negation, ∎ completion/finality, ∿ temporal/wave, "
        "@ focus/context, → flow/transformation, | boundary/pause, "
        "⊕ merge/union, ⊗ cross/tension, ⊂ subset/filter, ∅ emptiness/void.\n"
        "Compositional operators: A@B = B-aspect of A (projection), "
        "A*B = emergent meaning from A and B together (interference), "
        "A^0.7 = A at 70%% intensity (gradient), "
        "A\\B or A without B = A with B removed (subtraction), "
        "A±B = both A and B simultaneously (superposition), "
        "A:B = A given context B (conditional).\n"
        ...
        "lov=love, fer=fear, joy=joy, sor=sorrow, hot=hot, was=past, "
        "now=present, wil=future/will, dar=dark, lig=light.\n\n"
        f"Limn: {thought}\n\n"
