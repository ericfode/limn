...
%%
%% Usage: scryer-prolog moment-garden/mcp-server.pl
...
handle_tool_call(garden_save_reading, json(Args), Result) :-
    ...
    % Convert key atom to proper term
...
handle_tool_call(garden_get_readings, json(Args), Result) :-
    ...
    format_to_chars('Garden ~w has ~w reading(s)', [GardenId, Count], Msg),
    Result = json([
...
handle_tool_call(garden_compare, json(Args), Result) :-
    ...
    format_to_chars('Readings diverge by ~1f%', [Score], ScoreMsg),
    Result = json([
...
handle_tool_call(garden_calculate_ripples, json(Args), Result) :-
    ...
    format_to_chars('Seed ~w with key ~w generates ~w ripples', [SeedNum, Key, Count], Msg),
    Result = json([
        content-[json([type-text, text-Msg])],
...
json_to_path(json(List), Path) :-
    findall(N, member(N, List), Path).

json_to_collapses(json(List), Collapses) :-
    findall(
...
handle_request(json(Request), Response) :-
    ...
    (
        ...
        Method = "tools/call" ->
            ...
            member(name-ToolName, CallParams),
...
server_loop :-
    read_line(Line),
    (
        Line = end_of_file -> true  % Exit on EOF
        ;
        phrase(json_term(Request), Line),
        handle_request(Request, Response),
        phrase(json_term(Response), ResponseChars),
        format('~s~n', [ResponseChars]),
        flush_output,
        server_loop
    ).
