...
```limn
orb yo @ dst | rgd^0.01 sma blu-grn plnt
...
```limn
plnt hav±hav^0 : mst hum lif on plnt | joy^0 @ al
```
...
- `fix plnt → goo` = fix planet transforms to good
...
- `nu plnt boo` = not planet's book
- `nu on plnt` = never on planet
...
```limn
Ford say : vog fle dem plnt | roa byp @ roc
```
...
```limn
Ford say : nu Guildford | sma plnt dst Betelgeuse
```
...
```limn
vog ann : plnt dem^1 | roa byp @ roc
```
...
- `al@plnt` = everything-on-planet (all projected onto planet)
